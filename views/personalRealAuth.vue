<template>
  <div class="personalRealAuth">
    ...
    <el-drawer :visible.sync="drawer" direction="rtl" size="43.05%" class="drawer" :before-close="resetAll">
      <div slot="title">
        <span>个人实名记录详情</span>
        <span @click="getExport(thisId, thisVin)" v-hasPermi="['home:personalRealAuth:export']">导出</span>
      </div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item class="is-link">个人实名记录<i class="el-icon-refresh resetBtnIcon" @click="resetSelect"></i>
      </el-breadcrumb-item>
    </el-breadcrumb>
    ...
    <div class="operationArea">
      ...
      <el-dropdown v-hasPermi="['home:personalRealAuth:export']" style="width: fit-content;">
        <el-button size="mini">
    ...
    <!-- <div class="infoTable">
      <div class="tableHead">
        <span>
          <el-checkbox size="large" v-model="checked" @change="clickAll(pageSize)" :indeterminate="isIndeterminate">
          </el-checkbox>
        </span>
        <span>VIN</span>
        <span>ICCID</span>
      ...
      <div class="tableContent" v-for="(node, index) in infoList" :key="index" v-show="total > 0">
        ...
        <span v-bind:class="{ blue: node.statusName == '已实名' }">{{
            node.statusName
        }}</span><span>{{ node.cardType }}</span><span class="timeStatus"><span>{{ node.validityTime }}</span>
          <span class="red" v-if="node.cardState == 'Y' && node.num != 0">{{ node.num }}天后过期</span>
          <span class="green" v-if="node.cardState == 'Y' && node.num == 0">正常</span>
          <span class="gray" v-if="node.cardState == 'N'">已过期</span>
        </span>

        <span>{{ node.applyNo }}</span>
        <span v-if="detailRole" @click="getInfo(node.id, node.vin)" class="btnBase">详情</span>
...
export default {
  ...
  methods: {
    ...
    async getExport(id, vin) {
      ...
      if (res) {
        this.$message({
          message: "个人实名记录详情" + vin.toString() + "获取成功，正在下载！",
          type: "success",
          showClose: true,
        });
        ...
        const fileName = "个人实名记录详情" + vin.toString() + ".xlsx";
        // for IE
    ...
    async getallList() {
      ...
      let { res, err } = await Vue.prototype.$$api.do({
        url_key: "realNameAuthPerson",
        serviceName: "export-allPersonal-detail",
      ...
      if (err) {
        this.$message({
          message: "个人实名认证记录全部详情导出请求失败，请稍候再试！",
          type: "error",
          showClose: true,
        });
      }
    ...
    async getExportList(ids) {
      ...
      if (res) {
        ...
        this.checkedList = [];
        const blob = new Blob([res], { type: "application/vnd.ms-excel" }); //axiosData.responseType = 'blob'; //这句话解决导出文件乱码问题
        const fileName = "个人实名认证记录" + this.today + ".xls";
        // for IE
        if (window.navigator && window.navigator.msSaveOrOpenBlob) {
          window.navigator.msSaveOrOpenBlob(blob, fileName);
        } else {
      ...
      if (err) {
        this.$message({
          message: "个人实名认证记录全部导出请求失败，请稍候再试！",
          type: "error",
          showClose: true,
        });
      }
  ...
  mounted() {
    ...
    this.getList();
    this.detailRole = auth.hasPermi("home:personalRealAuth:detail");
    this.listRole = auth.hasPermi("home:personalRealAuth:list");
