<template>
  <div class="unitRealAuth">
    ...
    <el-drawer :visible.sync="drawer" direction="rtl" size="43.05%" class="drawer" :before-close="resetAll">
      <div slot="title">
        <span>单位实名认证详情</span>
        <span @click="getExport()" v-hasPermi="['home:unitRealAuth:export']">导出</span>
      </div>
      ...
      <div>
        ...
        <div class="infoDialog" v-show="infoDetail.name">
          <p></p>
          <span>车主信息</span>
        ...
        <div class="infoDialog" v-show="infoDetail.wtName">
          <p></p>
          <span>委托人信息</span>
          <img src="../assets/images/eyes.png" v-show="false" />
          <div>
        ...
        <div class="infoDialog orgInfoDialog" v-show="infoDetail.orgName">
          <p></p>
          <span>单位登记信息</span>
          <img src="../assets/images/eyes.png" v-show="false" v-if="orgInfoStatus" @click="changeEyes('O')" />
          <img src="../assets/images/eyesClose.png" v-show="false" v-if="!orgInfoStatus" @click="changeEyes('O')" />
          <div>
            <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item class="is-link">单位实名记录<i class="el-icon-refresh resetBtnIcon" @click="resetSelect"></i>
      </el-breadcrumb-item>
    </el-breadcrumb>
    <div class="selectArea">
      <div>
        ...
        <el-input v-model="orgName" placeholder="单位名称" v-bind:disabled="!listRole" suffix-icon="el-icon-search"
          @change="getList"></el-input>
        <el-input v-model="orgCode" placeholder="统一社会信用代码" v-bind:disabled="!listRole" suffix-icon="el-icon-search"
          @change="getList"></el-input>
        <el-input v-model="handlerPhone" placeholder="责任人号码" v-bind:disabled="!listRole" suffix-icon="el-icon-search"
          @change="getList"></el-input>
    ...
    <div class="operationArea">
      ...
      <el-dropdown v-hasPermi="['home:unitRealAuth:export']" style="width: fit-content;">
        <el-button size="mini">
        ...
        <el-dropdown-menu slot="dropdown">
          ...
          <el-dropdown-item @click.native="getExportList('or', 'realNameAuthRecord', 'export-list')">全部导出
          </el-dropdown-item>
          <el-dropdown-item @click.native="getallList">全部详情导出
          </el-dropdown-item>
        </el-dropdown-menu>
      </el-dropdown>
    </div>
    <el-table :data="infoList" border style="width: 100% ;height:fit-content;padding-bottom: 7px;" ref="multipleTable"
      ...
      <el-table-column width="240" align="center">
        <template slot="header">
          ...
          <img src="../assets/images/closeEyes.png" v-show="false" v-if="!orgCodeStatus" @click="changeHeadEyes('O')" />
        </template>
      ...
      <el-table-column width="180" align="center">
        <template slot="header">
          责任人姓名<img src="../assets/images/openEyes.png" v-show="false" v-if="handlerNameStatus == 1"
            @click="changeHeadEyes('HN')" />
          <img src="../assets/images/closeEyes.png" v-show="false" v-if="handlerNameStatus == 0"
            @click="changeHeadEyes('HN')" />
        </template>
        <template slot-scope="scope">
      ...
      <el-table-column fixed="right" label="操作" width="110" align="center">
        <template slot-scope="scope">
          <span v-if="unitdetailRole" @click="openUnitDialog(scope.row)" class="btnBase blueBtn">详情</span>
          <span v-if="!unitdetailRole" class="blueBtn disableBtn">详情</span>
    ...
    <!-- <div class="infoTable">
      ...
      <div class="tableContent" v-for="(node, index) in infoList" :key="index" v-show="total > 0">
        ...
        <span @click="openUnitDialog(node)" v-if="unitdetailRole" class="btnBase">详情</span>
        <span class="disableBtn" v-if="!unitdetailRole">详情</span>
      </div>
...
export default {
  ...
  methods: {
    ...
    changeHeadEyes(type) {
      if (type == 'HN') {
        if (this.handlerNameStatus == 0) {
          this.handlerNameStatus = 1;
        }
        else {
          this.handlerNameStatus = 0;
        }
      }
      else if (type == 'HP') {
    ...
    async deleteNode() {
      ...
      let { res, err } = await Vue.prototype.$$api.do({
        url_key: "realNameAuthRecord",
        serviceName: "remove",
        serviceParameter: { id: this.selectId },
        method: "get",
      });
      if (err) {
        console.log("err", err);
    ...
    async getExport() {
      ...
      if (res) {
        this.$message({
          message: "单位实名认证详情" + this.selectVin + "获取成功，正在下载！",
          type: "success",
          showClose: true,
        });
        ...
        const fileName = "单位实名认证详情" + this.selectVin + ".xlsx";
        // for IE
    ...
    async getallList() {
      ...
      let { res, err } = await Vue.prototype.$$api.do({
        url_key: 'realNameAuthRecord',
        serviceName: 'export-allCompany-detail',
      ...
      if (res) {
        console.log("getExport", res);
        this.$message({
          message: '单位实名认证记录' + this.today + "获取成功，正在下载！",
          type: "success",
          showClose: true,
        });
        const blob = new Blob([res], { type: "application/vnd.ms-excel" });
        // axiosData.responseType = 'blob'; //这句话解决导出文件乱码问题
        const fileName = '单位实名认证记录' + this.today + ".zip";
        // for IE
        if (window.navigator && window.navigator.msSaveOrOpenBlob) {
          window.navigator.msSaveOrOpenBlob(blob, fileName);
        } else {
      ...
      if (err) {
        this.$message({
          message: "单位实名认证记录全部详情导出请求失败，请稍候再试！",
          type: "error",
          showClose: true,
        });
      }
    ...
    async getExportList(ids, key, name) {
      ...
      if (res) {
        ...
        const blob = new Blob([res], { type: "application/vnd.ms-excel" });
        // axiosData.responseType = 'blob'; //这句话解决导出文件乱码问题
        const fileName = fileType + this.today + ".xls";
        // for IE
        if (window.navigator && window.navigator.msSaveOrOpenBlob) {
          window.navigator.msSaveOrOpenBlob(blob, fileName);
        } else {
      ...
      if (err) {
        this.$message({
          message: "单位实名认证记录全部导出请求失败，请稍候再试！",
          type: "error",
          showClose: true,
        });
      }
    ...
    async getList() {
      ...
      let params = {
        ...
        handlerNameStatus: this.handlerNameStatus,
    ...
    openUnitDialog(node) {
      this.drawer = true;
      this.getInfo(node.id, "realNameAuthRecord", "query-detail");
      this.selectId = node.id;
      this.selectVin = node.vin;
    },
  },
  created() { },
  computed: {
  ...
  mounted() {
    ...
    this.getList();
    this.unitdetailRole = auth.hasPermi("home:unitRealAuth:detail");
    this.listRole = auth.hasPermi("home:unitRealAuth:list");
