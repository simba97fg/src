<template>
  <div class="realNameUnbind">
    <el-breadcrumb separator-class="el-icon-arrow-right">
      <el-breadcrumb-item>其他业务管理</el-breadcrumb-item>
      <el-breadcrumb-item class="is-link">实名信息解绑<i class="el-icon-refresh resetBtnIcon" @click="resetSelect"></i>
      </el-breadcrumb-item>
    </el-breadcrumb>
    ...
    <el-dialog :visible.sync="dialogVisible" width="29.5%" center :show-close="false" style="padding-top: 15%">
      <p>确认进行测试期解绑（共 {{ vinList.length }} 张）</p>
      <p>此操作不可撤销</p>
      <span slot="footer" class="dialog-footer">
        <el-button @click="dialogVisible = false">取 消</el-button>
        <el-button type="primary" @click="removeCardInfo">确 定</el-button>
      </span>
    </el-dialog>
    <el-drawer :visible.sync="updateDrawer" direction="rtl" size="43.05%" class="updateDrawer"
      ...
      <div>
        ...
        <el-cascader :show-all-levels="true" v-model="selectedDept" :options="deptOptions"
          :props="memberDrawerDefaultProps"></el-cascader>
        <div>
          <span></span>
          <span>选择解绑后的车辆状态</span>
        </div>
        <el-select v-model="carStatus" class="margin65">
        ...
        <div v-if="updateStatus != '0'">
          ...
          <div>
            ...
            <span v-if="updateStatus == '2'" class="successP">解绑成功</span>
            <span v-if="updateStatus == '3'" class="errorP">解绑失败，请重试</span>
        ...
        <el-tooltip placement="bottom-start" effect="light" popper-class="popperclass">
          ...
          <span v-show="updateStatus == '2'" class="greenBtn">查看已解绑车辆</span>
        </el-tooltip>
        <el-tooltip placement="bottom-start" effect="light" popper-class="popperclass">
      ...
      <div>
        <el-divider></el-divider>
        <span v-if="updateStatus == '0'">确认解绑</span>
        <span v-if="updateStatus == '1'">解绑中</span>
        <span class="clickAbleBtn" v-if="updateStatus == '2'" @click="reStart">继续解绑</span>
        <span class="clickAbleBtn" @click="reStart" v-if="updateStatus == '3' || updateStatus == '4'">重新解绑</span>
        <span class="clickAbleBtn" v-if="updateStatus == '5'" @click="updatedialogVisible = true">确认解绑</span>
      </div>
    </el-drawer>
    <el-drawer :visible.sync="drawer" direction="rtl" size="40.27%" class="drawer">
      ...
      <div>
        <div class="baseInfo" v-if="infoDetail.baseData">
          ...
          <div>
            <span>解绑操作人</span><span>{{ infoDetail.baseData.operatorName }}</span>
          </div>
          <div>
            <span>解绑时间</span><span>{{ infoDetail.baseData.unboundTimeStr }}</span>
          </div>
        </div>
        <div class="listArea" v-if="infoDetail.baseData">
          ...
          <div>
            <div>
              <div v-for="(node, index) in infoDetail.iccdListData" :key="index">
                <div>
                  <span>{{ index + 1 }}</span>
                  <span>{{ node }}</span>
                </div>
    ...
    <div class="selectArea">
      <div>
        ...
        <el-input v-model="queryParams.batchNo" placeholder="批次号" v-bind:disabled="!listRole" @change="getList"
          suffix-icon="el-icon-search"></el-input>
        <el-select v-model="queryParams.operatorName" filterable placeholder="解绑操作人" v-bind:disabled="!listRole"
        ...
        <el-date-picker v-bind:disabled="!listRole" v-model="time" value-format="yyyy-MM-dd HH:mm:ss"
          prefix-icon="el-icon-date" type="daterange" start-placeholder="解绑起始时间" end-placeholder="结束时间" align="right">
        </el-date-picker>
      ...
      <span @click="resetSelect" class="resetBtn" v-bind:class="{ 'disabledBtn': !listRole }">重置</span> -->
        <span @click="openUpdateDrawer" v-show="changeRole" class="operBtn">去解绑</span>
      </div>
    ...
    <div class="infoTable">
      <div class="tableHead">
        ...
        <span>批次号</span>
        <span>卡数</span>
        <span class="timeStatus">解绑时间<i class="el-icon-caret-top" v-bind:class="{ blueBtn: queryParams.timeDesc == 1 }"
...
export default {
  ...
  methods: {
    ...
    upload() {
      if (this.carStatus && this.selectedDept != null) {
    ...
    async uploadfile() {
      const api = this.$$api.doFormDataReq({
        ...
        serviceParameter: {
          ...
          status: this.carStatus.toString(),
        ...
        if (res) {
          ...
          if (res.businessCode == '1') {
            if (res.data.errorList.length > 0) {
              ...
              let { res, err } = await Vue.prototype.$$api.do({
                url_key: "realNameUnbind",
                serviceName: "query-unbound-detail",
                serviceParameter: { batchNo: this.batchNo },
                method: "get",
              });
              if (res) {
                this.vinList = res.data.iccdListData;
              }
              this.getList();
            }
    ...
    async getBox1() {
      ...
      let { res, err } = await Vue.prototype.$$api.do({
        url_key: "realNameBox",
        serviceName: "getUnboundOperatorNameComboBox",
        method: "get",
      });
      if (res) {
    ...
    async downloadtemplate() {
      ...
      if (res) {
        this.$message({
          message: "待实名解绑车卡模板" + "获取成功，正在下载！",
          type: "success",
          showClose: true,
        });
        const blob = new Blob([res], { type: "application/vnd.ms-excel" });
        // axiosData.responseType = 'blob'; //这句话解决导出文件乱码问题
        const fileName = "待实名解绑车卡模板" + ".xls";
        // for IE
    ...
    async getList() {
      ...
      let { res, err } = await Vue.prototype.$$api.do({
        url_key: "realNameUnbind",
        serviceName: "query-unbound-list",
  ...
  created() { },
  mounted() {
    this.listRole = auth.hasPermi("home:realNameUnbind:list");
    this.detailRole = auth.hasPermi("home:realNameUnbind:detail");
    this.changeRole = auth.hasPermi("home:realNameUnbind:change");
